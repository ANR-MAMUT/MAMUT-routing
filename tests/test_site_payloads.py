...
def test_generate_site_payloads_writes_problem_catalogs_instance_pages_and_history(tmp_path: Path) -> None:
    ...
    assert 'id="selectionName"' in workbench_html
    assert 'id="selectionCost"' in workbench_html
    ...
    assert 'class="route-view-render"' in workbench_js
    assert 'class="route-view-depot-opacity" type="range"' in workbench_js
    assert '<summary>Filters' not in workbench_js
