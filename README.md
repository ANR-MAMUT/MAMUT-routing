...
`site build` materializes its build-time caches incrementally: the ATF cache (`dist/atf-cache/`, TD schedule tables and arc-click sidecars for the materialized-model families, materialized in parallel per cache entry; `--atf-max-n`, default 400, and `--atf-jobs`, which caps the phase's memory as well as its speed since each worker holds one full ATF set) and the BKS route-geometry cache (`dist/route-geometry-cache/`, sha-pinned per BKS at every instance size, produced by the MAMUT-routing-tools road engine per city; `--route-geometry-jobs`, `--skip-route-geometry` to opt out). Route-geometry `auto` mode uses one worker because a large-city road graph can exceed 15 GiB; explicitly set a higher worker count only on hosts with sufficient memory. Before computing missing geometry, the build validates every referenced raw extract under the gitignored `osmdata/` directory and fetches missing, malformed, or undersized road datasets from the union of the published benchmark-sidecar bounds. The fetch uses the `road_cache` profile: Overpass returns only road classes consumed by the graph engine and skeleton coordinates for their referenced nodes, without unrelated POIs. Large cities use atomic tiled downloads with a persistent validated tile cache, so an interrupted request resumes without publishing a partial extract. Use `--no-fetch-missing-osm` for an offline partial build (affected pages fall back to straight route lines), or `--skip-route-geometry` to skip the phase entirely. Staging builds seed the cache from the active `dist` and materialize the remainder into the staging output. The standalone strict `site materialize-route-geometry` command accepts `--fetch-missing-osm` for pre-warming; `site materialize-atf` remains available separately.
